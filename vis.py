...
def main(unused_argv):
  ...
  tf.compat.v1.gfile.MakeDirs(FLAGS.vis_logdir)
  save_dir = os.path.join(FLAGS.vis_logdir, _SEMANTIC_PREDICTION_SAVE_FOLDER)
  tf.compat.v1.gfile.MakeDirs(save_dir)
  raw_save_dir = os.path.join(
      FLAGS.vis_logdir, _RAW_SEMANTIC_PREDICTION_SAVE_FOLDER)
  tf.compat.v1.gfile.MakeDirs(raw_save_dir)
  ...
  with tf.Graph().as_default():
    ...
    if FLAGS.min_resize_value and FLAGS.max_resize_value:
      ...
      resized_shape = tf.compat.v1.to_int32([tf.squeeze(samples[common.HEIGHT]),
                                   tf.squeeze(samples[common.WIDTH])])
      predictions = tf.squeeze(
          tf.compat.v1.image.resize_images(tf.expand_dims(predictions, 3),
                                 resized_shape,
