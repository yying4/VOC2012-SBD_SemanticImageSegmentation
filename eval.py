...
def main(unused_argv):
  ...
  tf.compat.v1.gfile.MakeDirs(FLAGS.eval_logdir)
  ...
  with tf.Graph().as_default():
    ...
    weights = tf.compat.v1.to_float(tf.not_equal(labels, dataset.ignore_label))
    ...
    metric_map['eval/%s_overall' % predictions_tag] = tf.compat.v1.metrics.mean_iou(
        ...
        weights=weights)
    # IoU for each class.
    one_hot_predictions = tf.one_hot(predictions, num_classes,on_value=None, off_value=None)
    one_hot_predictions = tf.reshape(one_hot_predictions, [-1, num_classes])
    one_hot_labels = tf.one_hot(labels, num_classes,on_value=None, off_value=None)
